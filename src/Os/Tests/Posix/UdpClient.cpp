...
int main(int /*argc*/, char */*argv*/[])
{
	...
	const char messageTo[] = "ping";
	char messageFrom[sizeof("pong")];

	SignalAction action = getSignalAction(sigNum);
	::sigaction(sigNum, &action, nullptr);

	os::UdpClient client{ serverAddress };
	{
		os::Socket socket = client.native();
		enableNonblock(socket);
		enableAsync(socket);
		registredSocket = &client.native();
	}
	...
	while (true)
	{
		std::cout << counter << std::endl;
		{
			...
			if (result < 0)
			{
				std::cerr << "client.send(\"" << messageTo << "\") error " << result
						<< std::endl;
				return EXIT_FAILURE;
			}
			std::cout << "client.send(\"" << messageTo << "\")" << std::endl;
		}

		{
			std::cout << "Wait signal..." << std::endl;
			const ::timespec time{ 5, 0 };
			::siginfo_t info;
			if (::sigtimedwait(&action.sa_mask, &info, &time) < 0 && errno == ETIMEDOUT)
			{
				continue;
			}
		}

		{
			...
			if (result < 0)
			{
				std::cerr << "client.receive() error " << result << std::endl;
			}
			std::cout << "client.receive() -> " << messageFrom << std::endl;
		}
	}
